...
class pastavchik_payController {
    ...
    getPastavchik = async (req, res, next) => {
        let pastavchikID = req.body.pastavchik_id;
        let umumiy = 0;
        let model = await register_supplierModel.findOne({
            attributes:[
                [sequelize.literal("SUM(CASE WHEN type = 2 THEN summa ELSE 0 END)"),'kirim'],
                [sequelize.literal("SUM(CASE WHEN type = 0 THEN summa ELSE 0 END)"),'chiqim'],
            ],
            where: {
                pastavchik_id: pastavchikID,
            },
            raw: true
        })
        console.log(model)
        if(model){
            if(model.kirim > model.chiqim){
                umumiy = model.kirim - model.chiqim;
            }else{
                umumiy = model.chiqim - model.kirim;
            }
        }
